...
## 3. Data (not redistributed)

Benchmark images are **not** committed to this repository (recommended, for
licensing reasons). Download them and place them in any directory, then **set
the two path constants at the top of `main.py`** so the pipeline can find them:

```python
IMAGE_DIR = "./images/Typical"   # folder holding the 4 typical images
BSD68_DIR = "./images/BSD68"     # folder holding the 68 BSD68 images
```

These two constants are the *only* thing the pipeline uses to locate images, so
any directory layout works — just point them at your copies. The example above
matches a layout like:

```
<images root>/
├── Typical/    Lena.*  Barbara.*  Baboon.*  Peppers.*   (color or grayscale)
└── BSD68/      test001.png … test068.png
```

Typical images are matched by filename keyword (case-insensitive), so
`lena.png`, `Baboon.tiff`, etc. are all found; extensions are auto-detected
(png / jpg / tiff / bmp).

**Sources.** USC-SIPI and other standard test images; BSD68 (the 68-image test
subset of the Berkeley BSDS300). Each dataset carries its own license, which is
